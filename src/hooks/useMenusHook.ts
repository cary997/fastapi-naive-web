/*
 * @Author: guoyl
 * @Date: 2023-06-06 04:40:17
 * @LastEditors: Cary
 * @LastEditTime: 2024-02-22 17:23:53
 * @FilePath: /fastapi-naive-web/src/hooks/useMenusHook.ts
 * @Descripttion:
 */
...
export function getMenusHook(routerMap: Array<RouteChildrenConfigsTable>) {
  ...
  routerMap.forEach((items) => {
    ...
    //过滤目录类型但是目录不包含任何页面
    if (currentMenu.menu_type === 1 && currentMenu["children"].length === 0) {
      return;
    }
