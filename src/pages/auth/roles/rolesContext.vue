<!--
 * @Author: guoyaolei
 * @Date: 2024-02-05 23:33:29
 * @LastEditors: Cary
 * @LastEditTime: 2024-02-26 03:07:25
 * @FilePath: /fastapi-naive-web/src/pages/auth/roles/rolesContext.vue
 * @Descripttion: 
-->
<template>
  <n-space vertical>
    <n-spin :show="loadingRef">
      ...
      >
        <template #header-extra>
          <div class="flex flex-row space-x-4">
            <n-tag strong :bordered="false">
              {{ $t("status.userCont") }}: {{ currentRolesData.user_count }}
            </n-tag>
        ...
        >
          ...
          <n-descriptions-item :label="t('rolesPage.rolesFormNickName')">
            {{ currentRolesData.nickname }}
          </n-descriptions-item>
          <n-descriptions-item :label="t('rolesPage.rolesFormStatus')">
            {{
          ...
          <n-descriptions-item :label="t('rolesPage.rolesFormDesc')" :span="3">
            <span class="text-primary">
              {{ currentRolesData.desc }}
            </span>
          </n-descriptions-item>
          <n-descriptions-item :label="t('action.create_at')" :span="1.5">
            {{ timestampFormat(currentRolesData.create_at) }}
          </n-descriptions-item>
          <n-descriptions-item :label="t('action.update_at')" :span="1.5">
...
const handleDelClick = () => {
  window.$dialog.warning({
    ...
    content:
      t("rolesPage.rolesDeleteMsg") + "—— " + currentRolesData.value.nickname,
