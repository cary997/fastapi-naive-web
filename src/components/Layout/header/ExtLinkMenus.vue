<!--
 * @Author: guoyl
 * @Date: 2023-06-19 22:15:44
 * @LastEditors: Cary
 * @LastEditTime: 2024-02-25 20:20:03
 * @FilePath: /fastapi-naive-web/src/components/Layout/header/ExtLinkMenus.vue
 * @Descripttion: 
-->
<template>
  <n-tooltip placement="bottom" trigger="hover" :z-index="0">
    <template #trigger>
      <n-popover trigger="click">
        <template #trigger>
          <NButton text>
            <NIcon size="20" class="mr-2 animate-pulse text-primary">
              <GridDots28Regular />
            </NIcon>
          </NButton>
        </template>
          ...
          <n-grid-item :span="1" v-for="item in wholeMenusExtlink">
            <div class="flex p-1 m-2 justify-center items-center">
              ...
              >
                <NAvatar
                  :round="true"
                  :src="item.meta?.icon ? item.meta?.icon : 'default.png'"
                  :size="48"
                  :fallbackSrc="theme === null ? lightLogo : darkLogo"
                />
                <n-text depth="2">
                  {{
                    language === "en" ? item.meta?.en_title : item.meta?.title
                  }}
                </n-text>
              </RouterLink>
            </div>
