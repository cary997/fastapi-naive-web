<!--
 * @Author: Cary
 * @Date: 2024-02-21 19:26:49
 * @LastEditors: Cary
 * @LastEditTime: 2024-02-23 19:09:50
 * @FilePath: /fastapi-naive-web/src/components/Layout/header/userProfile.vue
 * @Descripttion: 
-->
<template>
  <n-modal
  ...
  >
    <n-descriptions label-placement="left" :column="1" bordered>
      ...
      <n-descriptions-item :label="t('usersPage.tableData.email')">
        <div v-if="!showInpt">
          {{ currentUser.email }}
          <NButton type="info" size="tiny" @click="handelEditClick" text>
            <template #icon>
              <n-icon>
                <Edit20Regular />
              </n-icon>
      ...
      <n-descriptions-item :label="t('usersPage.tableData.phone')">
        <div v-if="!showInpt">
          {{ currentUser.phone }}
          <NButton type="info" size="tiny" text @click="handelEditClick">
            <template #icon>
              <n-icon>
                <Edit20Regular />
              </n-icon>
      ...
      <n-descriptions-item :label="t('usersPage.tableData.type')">
        {{
          currentUser.user_type == 1 ? $t("usersPage.tableData.local") : "ldap"
        }}
      </n-descriptions-item>
      ...
      <n-descriptions-item :label="t('usersPage.tableData.status')">
        {{
          currentUser.user_status ? $t("status.enable") : $t("status.disable")
        }}
      </n-descriptions-item>
    </n-descriptions>

    <template #action>
      <div v-if="emailRuleValue && phoneRuleValue && showInpt" class="flex justify-end">
