/*
 * @Author: guoyl
 * @Date: 2022-12-26 21:40:49
 * @LastEditors: Cary
 * @LastEditTime: 2024-02-26 18:06:56
 * @FilePath: /fastapi-naive-web/src/router/index.ts
 * @Descripttion:
 */
import {
  createRouter,
...
router.beforeEach((to: toRouteType, _from, next) => {
    ...
    to.matched.some((item) => {
      ...
      if (Title)
        document.title = `${Title} | ${
          language === "zh-CN" ? item.meta?.title : item.meta?.en_title
        }`;
      else
        document.title = (
          language === "zh-CN" ? item.meta?.title : item.meta?.en_title
  ...
  if (userInfo) {
    ...
    if (_from?.name) {
      ...
      // 刷新
      if (useMenusStoreHook().wholeMenus.length === 0 && to.path !== "/login") {
        initRouter().then((router: Router) => {
          ...
          if (route && route.meta?.title) {
            if (route.meta.menu_type === 2) {
              useTabsStoreHook().handleTags("push", getTagHook(route));
            }
          }
